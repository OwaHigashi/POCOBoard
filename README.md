...
- `🖥 全画面表示`
  右上のトグル。Display ウィンドウの全画面 ON/OFF（表示タブの
  フルスクリーンボタンと連動）。
- `ACCEPT`
...
上部エリアはこの 3 つだけで、それ以外はすべてタブに収まっています。
エフェクト・横スクロール・表示の各タブは縦にスクロールできるので、
モニタが低くてウィンドウが縮んでも下の方の操作が隠れません。
...
- 色タグ、サイズタグ、固定位置タグの挿入
- 速度 `x1..x5`
- `文字サイズ` 50〜500%（全体スケール、10% 刻み。100% = `marquee_size` どおり）
- `流す / 停止`
...
- `文字表示モード` — タブ最上段の `MARQUEE (横スクロール)` / `COMMENT (AI コメント応答)`
  ...
  AI 側からも `mode` 命令で切り替えられます。起動時のモードは
  `config.ini` の `text_mode`（既定 `comment`。横スクロールは読み逃すと消えるので、
...
- `COMMENT モード (おわくさ AI フィード)` 欄
  ...
  - 行間は `config.ini` の `comment_spacing_pct`（既定 40）。行の上下余白・折返し間・行同士の
  ...
  - `🧠 システムプロンプト編集…` — 動作中のおわくさ（POCOMon `scripts/owakusa.py`）の
    **システムプロンプトの人格部分（既定は執事）・名前・返答の文字数**を配信中に差し替えるダイアログ。
    `取得` で今の内容を取ってきて、書き換えて `設定` で次の返答から効きます（文字数の欄も `設定` を
    押すまで反映されません）。`既定に戻す` でおわくさ側の起動時の値に。人格に関係なく守らせる
    基本ルール（日本語のみ・絵文字・傷つけない・「◯◯になって」でキャラを演じる）はおわくさ側で
    固定で、`基本ルールを表示` で読めますが編集はできません。`おわくさ側にファイル保存` を ON にすると再起動後も残ります。
...
- 横補正モード（モード 1 / モード 2）— 演出の横補正とカメラの横引き
  ...
  `hstretch_mode`（既定 1 = 補正なし。実機では 2 を config.ini で指定）。
...
- クライアント一覧（表示名 / #ID / IP。IP はリバースプロキシ経由でも
  `X-Forwarded-For` から実クライアント IP を表示）
- 個別の `許可 / 拒否`（cookie の #ID 単位）
- `IP ✓ / IP 🚫` — IP 単位のブロック。プライベートブラウズやアプリ内
  ブラウザで cookie が変わり #ID が増える相手でも、IP ブロックなら
  まとめて止められます（`全員を許可` で IP ブロックも解除）
- `全員を許可 / 全員を拒否`
- 手動更新
...
- `JOIN / NAME / TALK / UPLOAD / ADMIN` などを色付き表示
  （各行に送信者の表示名と `#ID` が入ります）
- `#ID` をクリックすると、そのユーザーの ブロック / IP ブロック を
  その場で切り替えるダイアログが開きます
- ログ消去
...
- 画像・動画一覧
  ...
  - Pococha のアプリ内ブラウザではダウンロードが動きません（アプリ内ブラウザの制限）。スマホでは見出しに
    `↗ Safari / Chrome で開く` ボタンが出て、iOS は `x-safari-http(s)://`、Android は `intent://…;package=com.android.chrome`
    で外部ブラウザの起動を試みます。1.5 秒たっても切り替わらなければ URL をクリップボードにコピーし、
    右上メニューの「Safari で開く」「ブラウザで開く」を使うよう案内します（外部ブラウザを開けるかはアプリ側の許可次第）。
...
hstretch_mode = 1             ; 起動時の横補正モード (既定 1=補正なし / 実機は 2)
