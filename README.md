...
- 📷 カメラ表示 (USB / 仮想カメラ) — **既定で ON**
  ...
  - 実カメラの映像は 9:16 (720x1280) の縦型枠で切り出され、縦横同率で
    拡大されます（アスペクト比は変わりません）。`位置X / 位置Y / ズーム`
    で映像内の狙った場所を枠に合わせます。
  - ManyCam などの仮想カメラは映像が既に完成しているため、**既定で
    全体表示（クロップなし）**になります。`縦型 9:16 クロップ` の
    チェックでどちらも上書き可能で、選択はカメラごとに記憶されます
    （実行中のみ）。
  ...
  - `縦横補正` — キャプチャーボードが縦画面 (1080x1920) を横フレーム
    (1920x1080) に押し込んで送ってくる場合、フレームを縦横入替サイズへ
    引き伸ばし直してつぶれを解消します（カメラごとに記憶）。
- 縦型出力補正 — 出力先が実際には縦型 (1080x1920) なのに 1920x1080 の
  信号で駆動される構成向け。カメラ・写真・動画・エフェクト・飛ぶ文字・
  ピアノロールの**全描画**を 9:16 比率の仮想キャンバスで構成してから
  16:9 へ引き延ばして出力します。出力チェーンで圧縮され直すと正しい
  比率に戻ります（config: `display_portrait_stretch`）。
  補正中のカメラ映像は黒帯なしの**全面貼り付け**になります（カメラも
  同じ歪み系の信号なので、そのまま通せば出力先で元に戻るため）。
- 🎹 ピアノロール (USB MIDI)
