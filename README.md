...
- 📷 カメラ表示 (USB / 仮想カメラ) — **既定で ON**
  ...
  - `横引き延ばし` — カメラ映像を**縦はそのまま、横方向だけ中央軸を
    基準に**指定倍率へ引き延ばします（50〜400 %、既定 285 %）。
    横につぶれて届く映像の補正用で、100 % にすると無補正の
    レターボックス表示。画面からはみ出した左右は切れます
    （config: `camera_hstretch_pct`）。285 % は実機校正値 — 圧縮が
    入口（キャプチャ）と出口（縦型パネル）で二重にかかる構成では
    補正は一段分の二乗になります（1.688² ≈ 2.85。理想的な 16:9
    チェーンなら (16/9)² ≈ 3.16）。
...
- 演出の横補正（効果・文字・ピアノ） — POCOBoard が自分で描く層
  （エフェクト・飛ぶ文字・ピアノロール・写真・動画・待受タイトル）は
  出力チェーンの横圧縮を**一回だけ**受けるので、横に狭い仮想画面で
  構成してから横一杯に引き延ばして出力します（100〜400 %、既定
  169 % = カメラ補正 285 % の平方根 = 圧縮一段分）。これで最終画面で
  BOMB の円は丸く、ピアノロールは全 88 鍵が横幅いっぱいに並びます。
  カメラ映像は入口でも圧縮されている（二段 = 285 %）ため、この補正は
  通らずカメラ欄の `横引き延ばし` で別途補正します
  （config: `output_hstretch_pct`）。
- 🎹 ピアノロール (USB MIDI)
...
camera_hstretch_pct = 285     ; カメラ横引き延ばし率 (50..400)。100=無補正、285=実機校正値
output_hstretch_pct = 169     ; 演出(効果・文字・ピアノ等)の横補正 (100..400)。169=285の平方根
