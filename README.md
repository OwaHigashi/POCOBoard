...
- 📷 カメラ表示 (USB / 仮想カメラ) — **既定で ON**
  ...
  - `横引き延ばし` — カメラ映像を**縦はそのまま、横方向だけ中央軸を
    基準に**指定倍率へ引き延ばします（50〜400 %、既定 290 %）。
    横につぶれて届く映像の補正用で、100 % にすると無補正の
    レターボックス表示。画面からはみ出した左右は切れます
    （config: `camera_hstretch_pct`）。290 % は実機校正値 — 圧縮が
    入口（キャプチャ）と出口（縦型パネル）で二重にかかる構成では
    補正は一段分の約二乗になります（1.7² ≈ 2.9。理想的な 16:9
    チェーンなら (16/9)² ≈ 3.16）。
...
camera_hstretch_pct = 290     ; 横引き延ばし率 (50..400)。100=無補正、290=実機校正値
